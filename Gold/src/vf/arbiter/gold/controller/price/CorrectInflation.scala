...
import vf.arbiter.gold.model.enumeration.Metal
...
object CorrectInflation
{
	 ...
	 */
	def apply(originalPrice: Price, contractDate: LocalDate, metals: Set[Metal], referenceDuration: Days)
	         (implicit cPool: ConnectionPool, exc: ExecutionContext, apiKey: ApiKey) =
	{
		...
		if (metals.isEmpty || contractDate.isToday)
			TryFuture.successCatching(InflationCorrectedPrice.notCorrected(originalPrice, contractDate))
		else {
				...
				cPool
					.tryWith { implicit c =>
						...
						metals.toVector.map { metal =>
							metal -> dateRanges.map { dates =>
								metal.pricesIn(originalPrice.currency).averageDuring(dates).waitForResult()
							}
						}
					}
					.flatMapCatching { results =>
						...
						val (failures, metalPrices) = results
							.map { case (metal, prices) =>
								// Checks whether price check failed for either side
								prices.findMap { _.failure } match {
									// Case: Price check failed for one or both sides
									//       => Can't determine the rate of inflation
					...
					// Determines the final price and returns with encountered non-critical failures
						...
						val data = metalPrices.map { case (metal, prices) =>
							...
							val weightThen = prices.first.weightForMoney(originalPrice.amount)
							(metal, weightThen, originalPrice * correctionMod)
						}
						val corrected = InflationCorrectedPrice(originalPrice.amount, contractDate,
							data.map { case (metal, wt, _) => metal -> wt }.toMap,
							data.map { _._3.amount }.sum / data.size, originalPrice.currency)
