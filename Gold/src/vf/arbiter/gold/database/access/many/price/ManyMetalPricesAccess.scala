...
object ManyMetalPricesAccess extends ViewFactory[ManyMetalPricesAccess]
{
	// IMPLEMENTED	--------------------
	...
	/**
	  ...
	  */
	override def apply(condition: Condition): ManyMetalPricesAccess = _ManyMetalPricesAccess(Some(condition))
	
	
	// NESTED	--------------------
	
	private case class _ManyMetalPricesAccess(override val accessCondition: Option[Condition]) 
		extends ManyMetalPricesAccess
}
	...
	extends ManyRowModelAccess[MetalPrice] with FilterableView[ManyMetalPricesAccess] with Indexed
{
	...
	/**
	 * @return Access to price entries that are missing the price value (due to a software bug)
	 */
	def withoutPrice = filter(model.pricePerTroyOunceColumn <=> 0)
